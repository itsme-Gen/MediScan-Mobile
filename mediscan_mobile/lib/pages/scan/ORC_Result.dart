...
class _ORCResultPageState extends State<ORCResultPage> {
      ...
      children: [
        ...
        Container(
          decoration: BoxDecoration(
            ...
            border: Border.all(color: _isEditingEnabled ? Colors.grey.shade300 : Colors.grey.shade200),
          ),
          child: TextField(
            controller: _controllers[key],
            enabled: _isEditingEnabled,
            style: const TextStyle(fontSize: 14),
            decoration: InputDecoration(border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), hintStyle: TextStyle(color: Colors.grey[400])),
          ),
        ),
  ...
  void _scanAgain() => Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => const ScanIDPage()));
  void _searchRecords() => Navigator.of(context).pushNamed(SearchPage.routeName);
  void _next() => Navigator.of(context).push(MaterialPageRoute(builder: (context) => MedicalInformationPage(patientData: _extractedData)));
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Row(children: [_buildNavIcon(Icons.home_outlined, 'Dashboard', Dashboard.routeName), _buildNavIcon(Icons.camera_alt_outlined, 'Scan ID', ScanIDPage.routeName), _buildNavIcon(Icons.search, 'Search', SearchPage.routeName), _buildNavIcon(Icons.folder_open_outlined, 'Records', '/records'), _buildNavIcon(Icons.chat_bubble_outline, 'Assistant', '/assistant')]),
                      ],
                    ),
                  ),
                  Expanded(
                    child: SingleChildScrollView(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text('ORC Result', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: Colors.black87)),
                            const SizedBox(height: 8),
                            Text('Review and edit the extracted information before proceeding', style: TextStyle(fontSize: 14, color: Colors.grey[600], height: 1.4), textAlign: TextAlign.center),
                            ...
                            Container(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  const Text('Original Image', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
                                  const SizedBox(height: 4),
                                  const Text('Scanned patient ID Document', style: TextStyle(color: Colors.black54, fontSize: 12)),
                                  const SizedBox(height: 16),
                                  Container(
                                    width: double.infinity, height: 150,
                                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey.shade300)),
                                    child: ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(widget.originalImage, fit: BoxFit.cover, width: double.infinity, height: double.infinity)),
                                  ),
                                  const SizedBox(height: 16),
                                  Container(
                                    padding: const EdgeInsets.all(12),
                                    decoration: BoxDecoration(color: const Color(0xFFE8F5E8), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFF4CAF50).withOpacity(0.3))),
                                    child: const Row(children: [Icon(Icons.check_circle, color: Color(0xFF4CAF50), size: 20), SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('ORC Confidence', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12)), Text('Processing completed successfully', style: TextStyle(fontSize: 11, color: Colors.black54))]))]),
                                  ),
                                ],
                              ),
                            ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Extracted Information', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)), SizedBox(height: 2), Text('Verify the accuracy of the extracted data', style: TextStyle(color: Colors.black54, fontSize: 12))])),
                                      IconButton(onPressed: () => setState(() { _isEditingEnabled = !_isEditingEnabled; }), icon: Icon(_isEditingEnabled ? Icons.edit_off : Icons.edit, color: const Color(0xFF0B79FF), size: 20), tooltip: _isEditingEnabled ? 'Disable editing' : 'Edit Fields'),
                                    ],
                                  ),
                                  const SizedBox(height: 20),
                            ...
                            // Action Buttons
                            Row(children: [Expanded(child: OutlinedButton(onPressed: _scanAgain, style: OutlinedButton.styleFrom(side: BorderSide(color: Colors.grey.shade300), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.refresh, color: Colors.black54, size: 18), SizedBox(width: 8), Text('Scan Again', style: TextStyle(color: Colors.black54, fontWeight: FontWeight.w600))])))]),
                            const SizedBox(height: 12),
                            SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _searchRecords, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0B79FF), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.search, color: Colors.white, size: 18), SizedBox(width: 8), Text('Search Records', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600))]))),
                            const SizedBox(height: 12),
                            SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _next, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF4CAF50), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text('Next', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)), SizedBox(width: 8), Icon(Icons.arrow_forward, color: Colors.white, size: 18)]))),
                            const SizedBox(height: 16),
                            Text('Click "Search Records" to verify against existing patient database', style: TextStyle(fontSize: 12, color: Colors.grey[600]), textAlign: TextAlign.center),
                          ],
                        ),
                      ),
                    ),
